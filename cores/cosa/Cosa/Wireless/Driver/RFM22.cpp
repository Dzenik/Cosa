/**
 * @file Cosa/Wireless/Driver/RFM22.cpp
 * @version 1.0
 *
 * @section License
...
#include "Cosa/Wireless/Driver/RFM22.hh"
...
RFM22::RFM22(uint16_t net, uint8_t dev, 
...
void 
RFM22::IRQPin::on_interrupt(uint16_t arg)
{
  UNUSED(arg);

  // The interrupt handler is called on rising signal (RFM22:DIO0). 
...
int 
RFM22::send(uint8_t dest, uint8_t port, const iovec_t* vec)
{
...
int 
RFM22::send(uint8_t dest, uint8_t port, const void* buf, size_t len)
{
  iovec_t vec[2];
  iovec_t* vp = vec;
...
int 
RFM22::recv(uint8_t& src, uint8_t& port, void* buf, size_t len, uint32_t ms)
{
...
void 
RFM22::set_output_power_level(int8_t dBm)
{
  // Fix: High power level setting for RFM22HW
...
int 
RFM22::get_input_power_level()
{
  return ((-read(RSSI_VALUE)) >> 1);
}

int 
RFM22::get_temperature()
{
  set(STANDBY_MODE);
